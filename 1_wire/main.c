...
#define BUSES                                                                                                          \
	(OWI_PIN_0) //!< Buses to search.// If more than one devices, can be changed as #define BUSES   (OWI_PIN_0) |
	            //!(OWI_PIN_1) etc
...
// Prototypes of functions used in examples.
 ...
 *  on the buses defined by BUSES. It then tries to find DS1820
 *  device on a bus, and communicate with them to read temperature (DS1820)
...
int main(void)
{
	...
	PRR = ~(1 << PRUSART0); // In Atmel START code, PRR register is enabled by default to reduce the power
	                        // consumption.//Disable it to enable UART module
